...
"""
import uuid
import carla_submap_wrapper as csw
import numpy as np
import os
import sys
import copy
    ...
    if args.not_use_api:
        ...
        if 'semantic_lane' in args.other_params:
            ...
            if not os.path.exists(boun_info_path) or not os.path.exists(lane_info_path) :
                ...
                bound_info = np.load(boun_info_path,allow_pickle=True).item()
                lane_info = np.load(lane_info_path,allow_pickle=True).item()
            lane_ids = csw.get_lane_ids_in_xy_bbox(x, y,bound_info,query_search_range_manhattan=args.max_distance)
            ...
            local_lane_centerlines = [csw.get_lane_segment_centerline(lane_id, lane_info) for lane_id in lane_ids]
            polygons = local_lane_centerlines
            # z = am.get_ground_height_at_xy(np.array([[x, y]]), city_name)[0]

            if args.visualize:
                angle = mapping['angle']
                vis_lanes = [csw.get_lane_segment_polygon(lane_id, lane_info)[:, :2] for lane_id in lane_ids]
                t = []
        ...
        for index_polygon, polygon in enumerate(polygons):
            # the first line modified by gaoxin
            ...
            if 'semantic_lane' in args.other_params:
                ...
                lane_segment = csw.city_lane_centerlines_dict(lane_info, lane_id)
...
def argoverse_get_instance(lines, file_name, args):
    ...
    if args.do_eval:
        ...
        # add by gaoxin
        data_length = len(id2info['AGENT']) - 30
        for i, line in enumerate(id2info['AGENT'][data_length:]):
